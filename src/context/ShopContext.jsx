...
const ShopContextProvider = (props) => {
    ...
    const addToCart = async (itemId, size) => {
        ...
        const selectedSize = hasSize ? size : 'default';

        if (cartData[itemId]) {
            cartData[itemId][selectedSize] = (cartData[itemId][selectedSize] || 0) + 1;
        } else {
        ...
        try {
            await axios.post(`${backendUrl}/api/cart/add`, {
                userId: localStorage.getItem('userId'),  // Hoặc lấy từ state
                itemId,
                size: selectedSize
              });
        } catch (error) {
